...
// Utilizando el objeto Turno ------------------------------------------------------

            if (!swio) {
                Pturno = new Turno(std::to_string(turno),std::to_string(jug_actual),std::to_string(caOldJugador),std::to_string(vaDadoFinal),tipo_casilla,std::to_string(t.getCasilla(j[jug_actual-1].getCasilla_actual()).getNumeroCasilla()));
